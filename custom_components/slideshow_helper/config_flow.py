...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_options(self, user_input=None):
        ...
        options_schema = vol.Schema(
            {
                vol.Optional(
                    CONF_MIN_RATING, default=self.config_entry.options.get(CONF_MIN_RATING, 0)
                ): int,
                vol.Optional(
                    CONF_INCLUDE_TAGS, default=self.config_entry.options.get(CONF_INCLUDE_TAGS, "")
                ): str,
                vol.Optional(
                    CONF_EXCLUDE_TAGS, default=self.config_entry.options.get(CONF_EXCLUDE_TAGS, "")
                ): str,
                vol.Optional(
                    CONF_REFRESH_INTERVAL,
                ...
                vol.Optional(
                    CONF_CYCLE_INTERVAL,
                    default=self.config_entry.options.get(
                        CONF_CYCLE_INTERVAL, DEFAULT_CYCLE_INTERVAL
                    ),
                ): int,
            }
        )
