...
class SlideshowImageEntity(CoordinatorEntity, ImageEntity):
    ...
    def image_url(self) -> str | None:
        """Return direct URL generated by coordinator.
    ...
    async def async_image(self) -> bytes | None:
        ...
        if not current_path:
            _LOGGER.warning(
                "async_image called with no current_path; coordinator data keys=%s", list(data.keys())
            )
            return None
        ...
        def _read(path: str) -> bytes | None:
            ...
            return None
        try:
            data = await self.hass.async_add_executor_job(_read, current_path)
            if data is None:
